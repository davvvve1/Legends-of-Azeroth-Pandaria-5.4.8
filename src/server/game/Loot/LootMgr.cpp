...
class LootTemplate::LootGroup                               // A set of loot definitions for items (refs are not allowed)
{
    ...
    public:
        ...
        void Process(Loot& loot, uint32 lootmode, Player* player, LootRollPolicy policy = LootRollPolicy::Normal) const;
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot& loot, uint32 lootmode, Player* player, bool guaranteed) const
{
    ...
    // With no eligible equal-chance fallback, normalize the explicit group
    // to remove its empty-drop chance while preserving relative weights.
    if (guaranteed && !possibleLoot.empty())
    {
        LootStoreItemList equalLoot = EqualChanced;
        equalLoot.remove_if(LootGroupInvalidSelector(loot, lootmode, player));
        if (equalLoot.empty())
        {
            maxRoll = 0.0f;
    ...
    if (!possibleLoot.empty())                             // First explicitly chanced entries are checked
    {
        ...
        int32 roll = irand(1, guaranteed ? std::max<int32>(1, int32(maxRoll * precision)) : int32(maxRoll * precision));
...
void LootTemplate::LootGroup::Process(Loot& loot, uint32 lootmode, Player* player, LootRollPolicy policy) const
{
    if (policy == LootRollPolicy::AllDirectItems)
    {
        for (auto const* list : { &ExplicitlyChanced, &EqualChanced })
            for (LootStoreItem* item : *list)
                if (!LootGroupInvalidSelector(loot, lootmode, player)(item))
...
void LootTemplate::Process(Loot& loot, bool rate, uint32 lootmode, uint8 groupId, Player* player, LootRollPolicy policy) const
{
