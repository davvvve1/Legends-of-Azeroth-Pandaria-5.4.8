...
namespace
{
bool ResurrectAtInstanceEntrance(Player* player)
{
    Map* map = player->GetMap();
    if (!map || map->IsBattlegroundOrArena() ||
        !(map->IsDungeon() || map->IsRaid() || map->IsScenario()))
        return false;

    Position entrance;
    bool found = false;
    auto useDungeonEntrance = [&](lfg::LFGDungeonData const* dungeon)
    {
        if (!dungeon || dungeon->map != map->GetId() ||
            dungeon->difficulty != map->GetDifficulty() ||
            (dungeon->faction >= 0 && player->GetTeamId() != !dungeon->faction) ||
            (dungeon->x == 0.0f && dungeon->y == 0.0f && dungeon->z == 0.0f) ||
            !MapManager::IsValidMapCoord(map->GetId(), dungeon->x, dungeon->y, dungeon->z, dungeon->o))
            return false;

        entrance.Relocate(dungeon->x, dungeon->y, dungeon->z, dungeon->o);
        return true;
    };

    // Preserve the entrance of the queued wing/scenario when one is available.
    if (Group* group = player->GetGroup())
        found = useDungeonEntrance(sLFGMgr->GetLFGDungeon(sLFGMgr->GetDungeon(group->GetGUID())));

    if (!found)
        if (AreaTriggerStruct const* trigger = sObjectMgr->GetMapEntranceTrigger(map->GetId()))
        {
            entrance.Relocate(trigger->target_X, trigger->target_Y, trigger->target_Z, trigger->target_Orientation);
            found = MapManager::IsValidMapCoord(map->GetId(), entrance);
        }

    // Scenarios often have no entrance portal; use their configured LFG start.
    if (!found)
        for (uint32 dungeonId : sLFGMgr->GetDungeonsByRandom(0))
            if (useDungeonEntrance(sLFGMgr->GetLFGDungeon(dungeonId)))
            {
                found = true;
                break;
            }
    ...
    // Teleport within the existing instance, without resetting its progress.
    // A rejected teleport must leave the normal spirit-release path available.
    if (!player->TeleportTo(map->GetId(), entrance.GetPositionX(), entrance.GetPositionY(),
...
void WorldSession::HandleRepopRequestOpcode(WorldPacket& recvData)
{
    ...
    if (ResurrectAtInstanceEntrance(GetPlayer()))
        return;
