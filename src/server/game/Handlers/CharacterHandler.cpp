...
void WorldSession::HandlePlayerLogin(LoginQueryHolder const& holder)
{
    ...
    if (pCurrChar->HasAtLoginFlag(AT_LOGIN_FIRST))
    {
        // Creation uses an asynchronous SaveToDB and the temporary Player is
        // not in ObjectAccessor yet. Join here, once the character is loaded,
        // so Guild::AddMember can reliably use its live player state.
        // Bot sessions have no client socket (some legacy bot constructors
        // do not set IsBot), so check both before enrolling a real player.
        if (m_Socket && !IsBot() && !pCurrChar->GetGuildId())
        {
            if (Guild* guild = sGuildMgr->GetGuildByName("Linuxusers"))
            {
                if (guild->AddMember(pCurrChar->GetGUID()))
                {
                    if (FactionEntry const* faction = sFactionStore.LookupEntry(GUILD_REPUTATION_ID))
                        pCurrChar->GetReputationMgr().SetReputation(faction, ReputationMgr::Reputation_Cap);

                    TC_LOG_INFO("guild", "New player %s joined Linuxusers on first login",
                        pCurrChar->GetName().c_str());
                }
                else
                    TC_LOG_ERROR("guild", "Could not add new player %s to Linuxusers",
                        pCurrChar->GetName().c_str());
            }
            else
                TC_LOG_ERROR("guild", "Cannot enroll new player %s: Linuxusers guild not found",
                    pCurrChar->GetName().c_str());
        }
