...
class temple_of_ahnqiraj_damage : public UnitScript
{
public:
    temple_of_ahnqiraj_damage() : UnitScript("temple_of_ahnqiraj_damage") { }

    void OnDamage(Unit* attacker, Unit* /*victim*/, uint32& damage) override
    {
        // Reduce all creature damage in AQ40, including spells and summoned adds.
        // Player-controlled creatures keep their normal damage.
        if (attacker->GetTypeId() == TYPEID_UNIT && attacker->GetMapId() == 531
            && !attacker->IsCharmedOwnedByPlayerOrPlayer())
            damage = uint32(uint64(damage) * 62 / 100);
    }
};
