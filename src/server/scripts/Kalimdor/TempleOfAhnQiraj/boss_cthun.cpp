...
namespace
{
void ReduceCthunPlayerDamage(Unit* victim, uint32& damage)
{
    if (!victim || victim->GetTypeId() != TYPEID_PLAYER)
        return;

    // Keep damage proportional to the original hit, but prevent any one
    // boss/tentacle hit from killing a full-health player. Consecutive hits
    // can still kill, so healing and avoiding Dark Glare remain necessary.
    damage = std::min(damage / 2, victim->CountPctFromMaxHealth(35));
}
}
...
class boss_eye_of_cthun : public CreatureScript
{
    public:
        ...
        struct eye_of_cthunAI : public ScriptedAI
        {
            ...
            {
                ReduceCthunPlayerDamage(victim, damage);
            }
            ...
            void Reset() override
            {
                ...
                // The raid enters together. Allow the bot formation to reach
                // its spread positions before the first chaining beam.
                // Subsequent beams (including later green phases) remain 3s.
                BeamTimer = 20000;
